CREATE OR REPLACE TASK INSERT_ROWS_STG_CLEANED
WAREHOUSE = COMPUTE_WH
SCHEDULE = '30 MINUTE'
AS
BEGIN
        INSERT INTO 
            cleaned_earthquake_history 
        SELECT * 
        FROM(
                        SELECT
                            ...
                            f.value:properties:code::varchar as code,
                            ...
                            f.value:geometry.coordinates[1]::float as longitute,
                        ...
                        FROM
                            ...
                            LATERAL FLATTEN(input => PARSE_JSON(json_value):features) AS f) as vp
                        WHERE 
                            rn = 1
                        AND NOT EXISTS(
                            SELECT 1 
                            FROM 
                                cleaned_earthquake_history ceh
                            WHERE 
                                ceh.code = vp.code
                            AND 
                                ceh.registered_at = vp.registered_at);
END;